...
T1 = length(unique(table2array(unique(data(:,3))))); % number of years
Y = reshape(table2array(data(:,7)),T1,N)'; % outcome
D = reshape(table2array(data(:,4)),T1,N)'; % treatment
D1 = reshape(table2array(data(:,5)),T1,N)'; % treatmentq
D2 = reshape(table2array(data(:,6)),T1,N)'; % treatmentd
